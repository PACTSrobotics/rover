...
void executeCommand() {
  ...
  if(c == MOTORLEFT){
     if(speed >= 127) {
       leftDrive.start(H_Bridge::FORWARD, map(constrain(speed,127, 255), 127, 255, 0, 255));
     } else {
       leftDrive.start(H_Bridge::BACK, map(constrain(speed, 0, 127), 127, 0, 0, 255));
     }
     
  }
  if(c == MOTORRIGHT){
     if(speed >= 127) {
       rightDrive.start(H_Bridge::FORWARD, map(constrain(speed,127, 255), 127, 255, 0, 255));
     } else {
       rightDrive.start(H_Bridge::BACK, map(constrain(speed, 0, 127), 127, 0, 0, 255));
     }
  }
